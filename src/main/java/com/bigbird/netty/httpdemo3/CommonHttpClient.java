...
/**
 * 基于Netty实现的通用的HTTP客户端工具，可以在实际开发中使用
 */
@Sharable
public class CommonHttpClient extends ChannelDuplexHandler implements HttpClient, InitClose {

    static Logger log = LoggerFactory.getLogger(CommonHttpClient.class);

    int maxResponseContentLength = 2 * 1024 * 1024;
    int workerThreads = 1;
    ...
    //自定义线程池
    NamedThreadFactory workThreadFactory = new NamedThreadFactory("netty_httpclient");
    ...
    //保存每一次的请求、响应信息 映射关系
    ConcurrentHashMap<String, ReqResInfo> dataMap = new ConcurrentHashMap<>();
        ...
        bootstrap.group(workerGroup).channel(NioSocketChannel.class)
                .handler(new ChannelInitializer<SocketChannel>() {
                    ...
                    protected void initChannel(SocketChannel ch) throws Exception {
                        ...
                        pipeline.addLast("handler", CommonHttpClient.this);
                        //CommonHttpClient本身就是一个ChannelHandler，同时支持出站、入站操作
                    }
                });
